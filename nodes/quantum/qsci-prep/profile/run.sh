...
#  0.  Paths & constants
# ═══════════════════════════════════════════════════════════════════════════════
INPUT_DIR  = os.environ.get("INPUT_DIR",  "/mf/input")
...
h1 = heff.astype(np.complex128)
h2 = eris.astype(np.complex128)
interaction_op = InteractionOperator(float(e_core), h1, h2)
qubit_hamiltonian: QubitOperator = jordan_wigner(interaction_op)

# Serialise as compact sparse JSON (matches OpenFermion .terms)
terms_json = []
for pauli_tuple, coeff in qubit_hamiltonian.terms.items():
    sorted_term = sorted(pauli_tuple)                 # sort by qubit index
    terms_json.append({
        "indices":    [int(q)        for q, _ in sorted_term],
        "operators":  [str(op)       for _, op in sorted_term],
        "coeff_real": float(coeff.real),
        "coeff_imag": float(coeff.imag),
    })

ham_json = {
    "format":  "openfermion-sparse",
    "n_qubits": n_qubits,
    "n_elec":  int(n_elec),
...
with open(f"{OUTPUT_DIR}/qubit_hamiltonian", "w") as fh:
    json.dump(ham_json, fh, indent=2)
print(f"[qsci-prep] Qubit Hamiltonian: {len(terms_json)} terms", flush=True)
