...
#   3. Load PySCF RHF mean-field from chkfile (HDF5)
#   4. Run Vayesta EWF with a single fragment over active_atoms
#      (fragmentation method, bath type, orbital filter are configurable)
#   5. Read dumped clusters.h5, extract heff + eris (chemist)
#   6. Convert eris chemist→physicist
#   7. Write T6 cluster Hamiltonian HDF5
#   8. Output e_mf (Ha) and n_active_orbitals
set -euo pipefail
...
# ── Run EWF decompose (Python heredoc) ────────────────────────────────────────
python3 << PYEOF
import os, sys
import numpy as np
import h5py
