#!/usr/bin/env bash
# =============================================================================
# build.sh — 构建 Cirq + qsim Docker 镜像
#
# 用法：
#   bash nodes/base_images/cirq/build.sh
#
# 环境变量：
...
echo
echo -e "${BLUE}${BOLD}══ Build Cirq + qsim Docker Image ══${NC}"
echo
...
# ── 2. 验证 ──────────────────────────────────────────────────────────────────
_info "验证 Cirq + qsim..."
if $DOCKER run --rm "${LOCAL_NAME}:${LOCAL_TAG}" python -c "import cirq, qsimcirq; print('OK')" 2>&1 | grep -q "OK"; then
    _ok "Cirq + qsim 验证通过"
else
    _warn "未能确认 Cirq 可用，请手动检查"
fi
