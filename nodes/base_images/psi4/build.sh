#!/usr/bin/env bash
# =============================================================================
# build.sh — 构建 Psi4 Docker 镜像（基于官方镜像）
#
# 用法：
#   bash nodes/base_images/psi4/build.sh
#
# 环境变量：
...
echo
echo -e "${BLUE}${BOLD}══ Build Psi4 Docker Image ══${NC}"
echo
...
# ── 2. 验证 ──────────────────────────────────────────────────────────────────
_info "验证 Psi4..."
if $DOCKER run --rm "${LOCAL_NAME}:${LOCAL_TAG}" python -c "import psi4; print(psi4.__version__)" 2>&1 | grep -q "1."; then
    _ok "Psi4 验证通过"
else
    _warn "未能确认 Psi4 可用，请手动检查"
fi
