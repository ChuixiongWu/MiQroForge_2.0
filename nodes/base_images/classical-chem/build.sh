#!/usr/bin/env bash
# =============================================================================
# build.sh — 构建 Classical Chemistry Docker 镜像
#
# 用法：
#   bash nodes/base_images/classical-chem/build.sh
#
# 环境变量：
...
echo
echo -e "${BLUE}${BOLD}══ Build Classical Chemistry Docker Image ══${NC}"
echo
...
# ── 2. 验证 ──────────────────────────────────────────────────────────────────
_info "验证 PySCF + Vayesta + OpenFermion..."
if $DOCKER run --rm "${LOCAL_NAME}:${LOCAL_TAG}" \
    python -c "import pyscf, vayesta, openfermion, openfermionpyscf, h5py, scipy; print('ok')" 2>&1 | grep -q "ok"; then
    _ok "PySCF + Vayesta + OpenFermion 验证通过"
else
    _warn "未能确认所有包可用，请手动检查"
fi
