#!/usr/bin/env bash
# orca-ccsd/profile/run.sh — CCSD/CCSD(T) 单点能量计算
set -euo pipefail
# MF2 init

mf_banner "orca-ccsd" "CCSD/CCSD(T) single-point energy calculation"
...
# ── 映射 frozen_core 关键字 ──────────────────────────────────────────────────
fc_kw=""
if [[ "${frozen_core}" == "false" ]]; then
    fc_kw="NoFrozenCore "
fi

echo "[orca-ccsd] ${METHOD}/${basis_set} FrozenCore=${frozen_core} Charge=${charge} Mult=${multiplicity} Cores=${n_cores}"

# ── 读取 stream input: xyz_geometry ──────────────────────────────────────────
XYZ_INPUT="${INPUT_DIR}/xyz_geometry"
if [[ ! -f "$XYZ_INPUT" ]]; then
    echo "[orca-ccsd][ERROR] Required stream input 'xyz_geometry' not found at ${XYZ_INPUT}" >&2
    exit 1
fi
cp "$XYZ_INPUT" "${WORKDIR}/input.xyz"
echo "[orca-ccsd] Loaded input geometry: $(wc -l < "${WORKDIR}/input.xyz") lines"
...
echo "[orca-ccsd] Generated input.inp:"
cat "${WORKDIR}/input.inp"

# ── 运行 ORCA ─────────────────────────────────────────────────────────────────
cd "$WORKDIR"
echo "[orca-ccsd] Running ORCA..."
/opt/orca/orca input.inp > output.log 2>&1
echo "[orca-ccsd] ORCA finished (${METHOD}). Parsing output..."

# ── 解析输出 ──────────────────────────────────────────────────────────────────
ENERGY=$(grep "FINAL SINGLE POINT ENERGY" output.log | tail -1 | awk '{print $NF}' || echo "")
if [[ -z "$ENERGY" ]]; then
    echo "[orca-ccsd][ERROR] Could not extract energy from output!" >&2
    cat output.log >&2
    exit 1
fi
...
if grep -q "SCF NOT CONVERGED" output.log; then
    CONVERGED="false"
    echo "[orca-ccsd][WARN] SCF DID NOT CONVERGE!"
fi

SCF_ITER=$(grep "SCF CONVERGED AFTER" output.log | tail -1 | awk '{print $4}' || echo "0")
echo "[orca-ccsd] Energy=${ENERGY} Ha  Converged=${CONVERGED}  SCF_iter=${SCF_ITER}"
...
if [[ -f "$GBW_FILE" ]]; then
    ...
    echo "[orca-ccsd][WARN] GBW file not found: ${GBW_FILE}" >&2
