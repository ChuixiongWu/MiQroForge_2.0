#!/usr/bin/env bash
# orca-casscf/profile/run.sh — CASSCF 单点能量计算
set -euo pipefail
# MF2 init

mf_banner "orca-casscf" "CASSCF single-point energy calculation"

echo "[orca-casscf] CASSCF/${basis_set} ActiveSpace(${active_electrons}e,${active_orbitals}o) Charge=${charge} Mult=${multiplicity} Cores=${n_cores}"

# ── 读取 stream input: xyz_geometry ──────────────────────────────────────────
XYZ_INPUT="${INPUT_DIR}/xyz_geometry"
if [[ ! -f "$XYZ_INPUT" ]]; then
    echo "[orca-casscf][ERROR] Required stream input 'xyz_geometry' not found at ${XYZ_INPUT}" >&2
    exit 1
fi
cp "$XYZ_INPUT" "${WORKDIR}/input.xyz"
echo "[orca-casscf] Loaded input geometry: $(wc -l < "${WORKDIR}/input.xyz") lines"
...
echo "[orca-casscf] Generated input.inp:"
cat "${WORKDIR}/input.inp"

# ── 运行 ORCA ─────────────────────────────────────────────────────────────────
cd "$WORKDIR"
echo "[orca-casscf] Running ORCA..."
/opt/orca/orca input.inp > output.log 2>&1
echo "[orca-casscf] ORCA finished. Parsing output..."

# ── 解析输出 ──────────────────────────────────────────────────────────────────
ENERGY=$(grep "FINAL SINGLE POINT ENERGY" output.log | tail -1 | awk '{print $NF}' || echo "")
if [[ -z "$ENERGY" ]]; then
    echo "[orca-casscf][ERROR] Could not extract energy from output!" >&2
    cat output.log >&2
    exit 1
fi
...
if grep -q "SCF NOT CONVERGED" output.log; then
    CONVERGED="false"
    echo "[orca-casscf][WARN] SCF DID NOT CONVERGE!"
fi

SCF_ITER=$(grep "SCF CONVERGED AFTER" output.log | tail -1 | awk '{print $4}' || echo "0")
echo "[orca-casscf] Energy=${ENERGY} Ha  Converged=${CONVERGED}  SCF_iter=${SCF_ITER}"
...
if [[ -f "$GBW_FILE" ]]; then
    ...
    echo "[orca-casscf][WARN] GBW file not found: ${GBW_FILE}" >&2
    echo "NO_GBW" > "${OUTPUT_DIR}/gbw_file"
fi

echo "[orca-casscf] Done."
